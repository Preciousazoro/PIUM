    ...
    <div className="flex min-h-screen bg-background text-foreground">
          ...
          <div className="max-w-7xl mx-auto space-y-8">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="text-lg sm:text-2xl font-bold">{withdrawals.length} Requests</span>
              </div>
            </div>
                            ...
                            <td className="px-4 py-4">
                              <div className="flex items-center gap-3">
                                {withdrawal.userId.avatarUrl ? (
                                  ...
                                  <div className="w-8 h-8 rounded-full bg-primary text-primary-foreground flex items-center justify-center font-medium text-xs flex-shrink-0">
                                    {withdrawal.userId.name.charAt(0).toUpperCase()}
                                  </div>
                                )}
                                <div className="min-w-0">
                                  <div className="font-medium text-sm truncate">{withdrawal.userId.name}</div>
                                  <div className="text-xs text-muted-foreground truncate">{withdrawal.userId.email}</div>
                                </div>
                              </div>
                            </td>
                            <td className="px-4 py-4">
                              <div>
                                <div className="font-semibold text-sm">{withdrawal.amount.toLocaleString()} TP</div>
                                <div className="text-xs text-muted-foreground">${withdrawal.convertedAmount.toFixed(2)} USD</div>
                              </div>
                            </td>
                            <td className="px-4 py-4">
                            ...
                            <td className="px-4 py-4">
                              <div className="flex items-center gap-1">
                                ...
                                <span className="text-xs">
                                  {withdrawal.withdrawalType === WithdrawalType.BANK_TRANSFER ? 'Bank' : 'Crypto'}
                                </span>
                              </div>
                            </td>
                            <td className="px-4 py-4">
                                ...
                                {withdrawal.status === WithdrawalStatus.PENDING && (
                                  <>
                                    <button
                                      ...
                                      className="p-1.5 hover:bg-green-500/10 rounded-lg transition-colors text-green-600 disabled:opacity-50"
                                    >
                                    ...
                                    <button
                                      ...
                                      disabled={updatingId === withdrawal._id}
                                      className="p-1.5 hover:bg-red-500/10 rounded-lg transition-colors text-red-600 disabled:opacity-50"
                                    >
                                      ...
                                      {withdrawal.withdrawalType === WithdrawalType.BANK_TRANSFER ? (
                                        <div className="space-y-2 text-xs">
                                          ...
                                          <div>
                                            ...
                                            <p className="font-medium mt-1">{withdrawal.bankDetails?.accountName}</p>
                                          </div>
                                          <div>
                                        ...
                                        <div className="space-y-2 text-xs">
                                          <div>
                                            ...
                                            <p className="font-medium mt-1">{withdrawal.cryptoDetails?.network}</p>
                                          </div>
                                          <div>
                                            <span className="text-muted-foreground">Wallet Address:</span>
                                            <p className="font-medium mt-1 break-all">{withdrawal.cryptoDetails?.walletAddress}</p>
                                          </div>
                                        </div>
                                      )}
                                    </div>
                                    ...
                                    {withdrawal.adminNote && (
                                      <div className="space-y-3">
                                        ...
                                        <div className="bg-muted/30 border border-border rounded-lg p-3">
                                          <p className="text-xs">{withdrawal.adminNote}</p>
                                        </div>
                                      </div>
                                    )}
                                  </div>
                                  ...
                                  {withdrawal.status === WithdrawalStatus.PENDING && (
                                    <div className="flex flex-col sm:flex-row gap-2 pt-3 border-t border-border">
                                      <button
                                        onClick={() => openActionModal(withdrawal, 'approve')}
                                        disabled={updatingId === withdrawal._id}
                                        className="flex-1 px-4 py-2 bg-green-500 text-white rounded-lg hover:bg-green-600 disabled:opacity-50 transition-colors text-sm font-medium"
                                      >
                                        {updatingId === withdrawal._id ? (
      ...
      {showNoteModal && selectedWithdrawal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="bg-card border border-border rounded-xl p-4 sm:p-6 max-w-md w-full max-h-[90vh] overflow-y-auto">
            ...
            <div className="space-y-4">
              ...
              <div className="flex flex-col sm:flex-row gap-3 pt-2">
                <button
                  ...
                  className="flex-1 px-3 sm:px-4 py-2 bg-muted text-foreground rounded-lg hover:bg-muted/80 transition-colors text-sm"
                >
                  Cancel
                </button>
                <button
                  ...
                  className="flex-1 px-3 sm:px-4 py-2 bg-primary text-primary-foreground rounded-lg hover:bg-primary/90 disabled:opacity-50 disabled:cursor-not-allowed transition-colors text-sm"
                >
